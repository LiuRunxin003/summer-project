...
def evaluate(probs, y_true, verbose=False, normalize=False, bins=15):
    ...
    Params:
        probs: np.ndarray of shape (n_samples, n_classes) – class probabilities
        y_true: true class labels
        verbose: whether to print metrics
        normalize: if True, normalize 1-vs-K probabilities
        bins: number of bins for ECE/MCE

    Returns:
        (error, ece, mce, loss, brier)
    """
    ...
    try:
        if probs.shape[1] == 2:
            y_prob_true = np.array([probs[i, y_true[i]] for i in range(len(y_true))])
            brier = brier_score_loss(y_true=y_true, y_prob=y_prob_true)
        else:
            brier = None  # 多分类时不适用
    except Exception as e:
        print(f"[警告] Brier Score 计算失败：{e}")
        brier = None
    ...
    if verbose:
        ...
        print("MCE:", mce)
        print("LogLoss:", loss)
        print("Brier:", brier if brier is not None else "N/A (not binary)")
